...
async def killerbio_loop():
    killerbioSTART = gvarstatus("killerbio") == "true"
    while killerbioSTART:
        DMY = time.strftime("%d.%m.%Y")
        HM = time.strftime("%H:%M")
        bio = f"🎩It's time to kill you {HM} 💀"
        LOGS.info(bio)
        try:
...
@catub.cat_cmd(
    pattern="killerbio$",
    command=("killerbio", plugin_category),
    info={
        ...
        "note": "To stop this do '.end killerbio'",
        "usage": "{tr}killerbio",
    },
)
async def _(event):
    "To update your bio along with time"
    if gvarstatus("killerbio") is not None and gvarstatus("killerbio") == "true":
        return await edit_delete(event, "`killerbio is already enabled`")
    addgvar("killerbio", True)
    await edit_delete(event, "`بیوگرافی گودرتی برا مستر کیلر عزیز ران شد😎💪`")
    await killerbio_loop()
...
async def _(event):  # sourcery no-metrics
    ...
    if input_str == "killerbio":
        if gvarstatus("killerbio") is not None and gvarstatus("killerbio") == "true":
            delgvar("killerbio")
            await event.client(
                functions.account.UpdateProfileRequest(about=DEFAULTUSERBIO)
            )
            return await edit_delete(event, "`بیوگرافی گودرتی برا مستر کیلر جونم ران شد😎`")
        return await edit_delete(event, "`بیوگرافی گودرتی ران نیست گلم😐💔`")
    if input_str == "spam":
...
catub.loop.create_task(killerbio_loop())
